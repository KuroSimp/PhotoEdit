...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initMultiSlider(){
        ...
        const prevButton = slider.querySelector('.pe-prev');
        const nextButton = slider.querySelector('.pe-next');
        ...
        function setNavDisabled(disabled){
            ...
            slider.classList.toggle('is-transitioning', disabled);
        }

        function goTo(newIndex){
            if (isTransitioning) return;
            if (performance.now() < navigationLockedUntil) return;
            if (!slides[0]) return;
            isTransitioning = true;
            navigationLockedUntil = performance.now() + transitionMs;
            setNavDisabled(true);
            index = newIndex;
        ...
        function handleNavigationClick(event, direction){
            ...
            if (isTransitioning || performance.now() < navigationLockedUntil) return;
            goTo(index + direction);
        }

        prevButton?.addEventListener('click', (event) => handleNavigationClick(event, -1), true);
        nextButton?.addEventListener('click', (event) => handleNavigationClick(event, 1), true);
